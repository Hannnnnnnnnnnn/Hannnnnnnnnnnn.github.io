...
- **`filter` 는 `mask` 보다 **먼저** 적용된다 — 마스크 모양을 바꾸려면 래퍼가 필요하다.**
  ...
  반투명 중간 상태를 뭉개기 때문.
- **`radial-gradient` 로는 원 말고 못 그린다** (회전 가능한 타원도 안 된다). 붓 자국 같은
  모양은 **작은 마크를 촘촘히 찍고 SVG 필터로 융합**해서 얻는다. 세 가지가 동시에 맞아야 한다:
  1. **간격이 촘촘해야 한다** (`STEP` 6px, 30점). 성기면 임계값을 넘지 못한 마크가 낱개로
     흩어져 점선이 된다 — 실제로 `STEP` 13px + 큰 반지름일 때는 획이 아니라 구름이 됐다.
  2. **굵기는 속도에서 나온다.** 고정 거리(`STEP`)를 지나는 데 걸린 시간이 곧 속도의
     역수이므로, 그 시간으로 마크 반지름을 정하면 빨리 그을수록 가늘어진다 — 실제 붓의 성질.
  3. **노이즈를 한 축으로 늘리면 붓털 결이 된다.** `baseFrequency="0.006 0.085"` 처럼 x 와 y
     를 크게 다르게 주는 것이 요점. 등방성 노이즈는 그냥 울퉁불퉁한 얼룩이 된다.
  꼬리가 점으로 흩어지면 반지름·알파의 **하한**을 올려라(`0.3 + 0.7*a` → `0.5 + 0.5*a`).
  임계값(`feColorMatrix` 알파 배수)이 너무 세도 옅은 마크가 통째로 사라진다.
