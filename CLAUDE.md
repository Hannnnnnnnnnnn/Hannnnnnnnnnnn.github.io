...
- **`mix-blend-mode: difference`는 아래에 반전할 것이 있을 때만 성립한다.** 커서 블롭을
  페이지 전체에 붙였더니, 여백 위에서는 흰 원이 흰 배경과 연산되어 **단단한 검은 원**이
  콘텐츠를 덮었다. 이 레이아웃은 대부분이 여백이라 사실상 항상 그 상태였고, 결함으로 보였다.
  참조로 삼은 사이트가 같은 블렌드를 쓰는 것은 그 페이지가 풀블리드 영상·꽉 찬 타이포로
  덮여 있어 항상 반전 대상이 있기 때문이다 — **효과만 가져오고 그 효과가 성립하는 조건을
  안 가져오면 깨진다.** 지금은 히어로 글자 상자 안에서만 표시하고, 경계는 그라데이션으로
  흐린다.
- **글자 위 판정은 `h1` 블록 상자가 아니라 낱자 상자의 합집합으로 한다.** `h1`은 가로 전체를
  채우므로 텍스트 오른쪽 빈 공간(1614→1864px)까지 "글자 위"로 잡힌다.
- **`radial-gradient`에 `closest-side`를 붙여라.** 기본값은 `farthest-corner`라 정사각형
  상자에서 그라데이션 끝이 상자 밖(≈1.41배)에 놓이고, 변에서 잘려 딱딱한 경계가 남는다.
