...
- **`filter` 는 `mask` 보다 **먼저** 적용된다 — 마스크 모양을 바꾸려면 래퍼가 필요하다.**
  ...
  반투명 중간 상태를 뭉개기 때문.
- **붓 질감은 코드가 아니라 스탬프 비트맵에서 나온다.** `feTurbulence` 같은 절차적 노이즈는
  "울퉁불퉁함"은 만들어도 붓털은 못 만든다. 초기화 때 붓끝 텍스처를 캔버스에 한 장 그려 두고
  진행 방향으로 회전시켜 반복해 찍는다(페인팅 앱의 방식). 스캔한 붓끝 PNG 로 갈아끼우면
  코드 수정 없이 그대로 좋아진다.
- **각져 보이는 것도 구슬처럼 이어져 보이는 것도 개별 스탬프가 아니라 겹침의 경계에서
  생긴다 — 합집합을 흐려야 한다.** 스탬프마다 회전을 무작위로 주면 이웃끼리 각도가 어긋나
  경계가 지그재그로 튀고, 반대로 회전을 아주 천천히 돌리면 이웃이 거의 같은 모양이 되어
  `STEP` 주기의 규칙적인 물결무늬(스캘럽)가 생긴다. **둘 다 개별 스탬프를 손봐서는 못 고친다.**
  스탬프를 오프스크린 캔버스에 모아 그린 뒤 그 결과를 한 번 `filter = "blur(9px)"` 로 얹으면
  경계가 통째로 녹아 액체가 된다. 페이드 알파도 이때 한 번만 적용한다.
- **꼬리부터 사라지면 획이 갉아먹히는 것처럼 보인다.** 마크마다 제 나이로 페이드시키면
  가장 오래된 쪽이 먼저 지워진다. 스트로크 전체가 **하나의 알파**를 공유하고 통째로 걷히게
  하면 훨씬 자연스럽다. 검증은 눈이 아니라 좌우 끝 위치로 — 균일 페이드면 페이드 중에도
  획의 좌우 범위와 왼쪽 끝이 그대로여야 한다(측정값 각각 1px 이동).
- **액체처럼 보이려면 무작위성을 정점이 아니라 위상에 줘야 한다.** 각도별 반지름에
  `Math.random()` 을 더하면 가장자리가 오톨도톨해져 액체가 아니라 부스러기로 보인다.
  매끄러운 하모닉(`sin(a*2+p1) + sin(a*3+p2) + sin(a*5+p3)`)만 쓰고 무작위성은 위상 `p` 에만
  주면 윤곽이 흐르는 곡선이 된다. 이음도 `lineTo` 가 아니라 **이웃 두 점의 중점을 지나는
  `quadraticCurveTo`** 로 해야 접선이 연속이라 다각형 티가 안 난다.
  마지막으로 캔버스 `filter = "blur(2.5px)"` 로 가장자리를 살짝 번지게 하면 종이에 스민
  젖은 경계가 된다(`"filter" in ctx` 로 지원 확인 후 사용).
- **완벽한 원은 잉크로 안 보인다 — 물방울무늬로 보인다.** 튀는 방울을 `arc()` 로 그렸더니
  화면 전체에 고른 동그라미가 흩뿌려져 장식 패턴처럼 읽혔다. 붓끝 비트맵을 작게 회전시켜
  재사용하면 방울도 불규칙해진다(코드도 줄고). 크기는 `r * r` 처럼 제곱으로 뽑아 작은 쪽에
  치우치게 하고, 날아간 방향으로 늘여야 한다. 획에서 너무 멀리 흩으면 관계가 끊겨 먼지처럼
  보이므로 거리도 획 굵기에 비례해 가깝게 묶는다.
- **이벤트가 온 지점에만 찍으면 획이 끊긴다 — 구간을 보간해야 한다.** `pointermove` 는
  커서를 빠르게 휘두르면 50~100px 씩 벌어져 도착하므로, 이벤트 위치에만 스탬프를 찍으면
  자국이 낱개로 흩어진다. 직전 점과 새 점 사이를 `STEP` 간격으로 채워 찍어야 이벤트 밀도와
  무관하게 이어진다. 보간을 넣으면 마크 밀도가 몇 배로 뛰므로 **개수 상한도 같이 올려야**
  한다(안 올리면 한 번 휘두르는 것만으로 이전 획이 통째로 밀려난다).
- **캔버스 + `mix-blend-mode: difference` 는 반전판 + 마스크와 결과가 같고 레이어가 셋 적다.**
  흰 자국을 difference 로 얹으면 흰 배경은 검게, 검은 글자는 희게 뒤집힌다. 단 블렌드가
  히어로 밖으로 새지 않도록 `.hero` 에 `isolation: isolate` 와 **불투명한 배경**을 줘야 한다
  (배경이 투명하면 블렌드 대상이 없어 결과가 어긋난다).
