...
**초록색 push는 배포 완료가 아니다.** 반드시 순서대로:

1. 빌드 상태를 먼저 묻는다 —
   `gh api repos/Hannnnnnnnnnnn/Hannnnnnnnnnnn.github.io/pages/builds --jq '.[0] | {status,commit:.commit[0:7],error:.error.message}'`
   `status`가 `building`이면 아직이다. 폴링 루프로 기다리지 말고 상태를 물어라.
2. 그 다음 실제 파일을 받아 **로컬과 바이트 대조**한다.
